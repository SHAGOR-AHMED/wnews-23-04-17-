<?php
	...
	$tableName="kha_news_category_info";
	$keyField="news_cat_id";
...
/**********Add Record*********************************************************************************/
	...
	{
			$id=$CodeSystem->MakeID("NCATID-",10,$tableName,$keyField);
			$sql="INSERT INTO $tableName(news_cat_id,news_cat_name,news_cat_status)
			VALUES
			( '".$id."',
			  '".$_POST["news_cat_name"]."',
			  '1'
...
/**********Edit Record*********************************************************************************/
	if(isset($_POST["btnEdit"]))
	{
		$sql="UPDATE $tableName SET 
			  news_cat_name='".$_POST["news_cat_name"]."'
			  WHERE $keyField = '".$_POST["news_cat_id"]."'";
		$result = mysql_query($sql);
...
/*********Delete Record****************************************************************/
	...
	{
		$sql="DELETE  FROM $tableName WHERE $keyField='".$_POST["news_cat_id"]."'";
		$result=mysql_query($sql);
...
<div class="box">
      <div class="box-header">
        <h3 class="box-title">News Category Information</h3>
      </div><!-- /.box-header -->
        <div class="box-body no-padding">
            <table class="table table-condensed">
                <tr>
                    ...
                    <td><label for="news_cat_id"></label>
                  <input name="news_cat_id" type="text" id="news_cat_id" readonly="readonly" value="<?php echo $row[0];?>" /></td>
                </tr>
                <tr>
                    <td>&nbsp;</td>
                    <td>News Category Name</td>
                    <td><label for="admin_user"></label>
                  <input type="text" name="news_cat_name" id="news_cat_name" value="<?php echo $row[1];?>" /></td>
                </tr>
                <tr>
